 ...
 */
{
    ...
    class Content {
        constructor(el) {
			...
			this.DOM.imgSupporting = this.DOM.el.querySelector('.content__item-supporting');
			console.log(this.imgSupporting);
    ...
    class Grid {
		...
		openItem(item) {
			...
			TweenMax.to(item.DOM.tilt.img, 1.2, {
				...
				onComplete: () => {
					...
					var supportingDiv = document.querySelector('.content__item-supporting');
					var supportingDivImage = document.querySelectorAll('.content__image-supporting');
					if(supportingDiv){
						supportingDivImage.forEach(function(extraImages) {
							extraImages.style.visibility = 'visible';
							extraImages.classList.add('fadein');
						});
					}
					// if(supportingDiv){
					// 	supportingDivImage.forEach(function(extraImages) {
		...
		closeItem(withAnimation = true) {
			...
			if(supportingDiv){
				supportingDivImage.forEach(function(extraImages) {
					extraImages.classList.remove('fadein');
					extraImages.classList.add('fadeout');
				});
			}
